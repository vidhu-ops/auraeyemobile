...
- **Image Size Validation**: All image uploads are limited to 3MB (3000KB) maximum across all endpoints (aura analysis, object analysis, vibe check, gemini analyze). Files exceeding this limit are rejected with error message showing exact file size and requesting smaller image upload: "Image size too large (X.XKB). Please upload a smaller size image (max 3MB)."
