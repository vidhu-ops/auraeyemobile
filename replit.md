...
- **Numerology**: Comprehensive calculations and interpretations including life path, destiny, and chakra analysis. Combined analysis tab features corrected personality number calculation using only day digits from birth date, reduced to single digit with proper color mapping. **Registration Integration**: Birth date is now required during client registration. Upon successful registration, clients immediately see their personal numerology blueprint with core numbers (Life Path, Destiny, Soul Urge, Personality) and spiritual interpretation before proceeding to onboarding preferences. This provides instant spiritual guidance and personalized insights based on their birth numbers.
...
- **Credit System**: Dynamic pricing based on user type. Clients: Object scan (3), Connect with healer (3), Human aura scan (15), What's my vibe (1), Numerology (5). Healers: Object scan (1), Human aura scan (5), What's my vibe (1), Healer booking (1), Numerology (3). New clients receive 30 initial credits, new healers receive 100 initial credits. **Payment Integration**: Single credit pack (₹499 for 100 credits) available via Stripe Payment Link. Webhook automatically adds credits to user account and upgrades client accounts to healer status upon successful payment. Idempotency protection using stripe_session_id prevents duplicate credit grants on webhook retries.
