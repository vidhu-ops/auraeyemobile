...
- **PDF Generation**: Comprehensive PDF reports for aura analysis (for clients) and detailed reports (for healers), including original and processed images, chakra analysis, spiritual guidance, and healer notes. Ultra-high quality settings: PDF compression set to maximum quality (1.0), screenshot PNG quality at 0.98. Enhanced tabs system with specific handling: Analysis tab (1st tab) and Energy Map tab captured in exactly 4 sections with 2 sections per page to prevent overlapping, with enhanced spacing (25px between sections). Life Score tab uses maximum settings (6.5x scaling, 50MB limit, 24px font, 800 weight). Other enhanced tabs (guidance, detailed analysis) use 5.6x scaling with 35MB limits. PDF layout: separate pages for each tab, intelligent spacing, section titles for analysis/energy-map tabs. Chakra details tab unchanged as requested. Enhanced text rendering with larger fonts and optimized legibility.
