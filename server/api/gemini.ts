...
export async function generateAuraVisualization(
  ...
  try {
    ...
    // Add zone labels
    addZoneLabels(ctx, STANDARD_WIDTH, STANDARD_HEIGHT);
...
function addDenseSmokeyClouds(
  ...
  for (let ring = 0; ring < cloudRings; ring++) {
    ...
    for (let cloud = 0; cloud < numClouds; cloud++) {
      ...
      let color;
      if (colorIndex === 0) color = primaryRGB;
      else if (colorIndex === 1) color = secondaryRGB;
      ...
      smokeGradient.addColorStop(0.2, `rgba(${color.r}, ${color.g}, ${color.b}, ${cloudOpacity * 0.9})`);
      smokeGradient.addColorStop(0.5, `rgba(${color.r}, ${color.g}, ${color.b}, ${cloudOpacity * 0.6})`);
      smokeGradient.addColorStop(0.8, `rgba(${color.r}, ${color.g}, ${color.b}, ${cloudOpacity * 0.2})`);
...
function addZoneLabels(ctx: any, canvasWidth: number, canvasHeight: number) {
