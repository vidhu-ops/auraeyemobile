...
const TEMPLATES: Record<ImportTarget, { headers: string[]; sample: Record<string, string | number>[] }> = {
  users: {
    ...
    sample: [{ username: "jane_client", password: "ChangeMe123", name: "Jane Client", email: "jane@example.com", mobileNumber: "", userType: "client", credits: 10, creditValidityDays: 30 }],
  },
  leads: {
    headers: ["name", "email", "mobileNumber", "source", "stage", "notes"],
    sample: [{ name: "Prospective Healer", email: "healer@example.com", mobileNumber: "", source: "csv_import", stage: "new", notes: "" }],
  },
};
  ...
  return (
    <div className={`rounded-xl border border-slate-200 bg-slate-50 p-3 space-y-2`}>
      <div className="text-sm font-medium flex items-center gap-2">
        ...
        Upload spreadsheet ({target})
      </div>
      <p className="text-[11px] text-slate-500">Download the template, fill it in Excel, then upload.</p>
      <div className="flex flex-wrap gap-2">
        <Button size="sm" variant="outline" onClick={downloadTemplate} disabled={disabled}>
          Download template
        </Button>
        <label className={`inline-flex items-center ${disabled || busy ? "opacity-50 pointer-events-none" : "cursor-pointer"}`}>
          <input type="file" accept=".csv,.xlsx,.xls" className="hidden" disabled={disabled || busy} onChange={(e) => onFile(e.target.files?.[0] || null)} />
          <span className="inline-flex h-8 items-center rounded-md border border-slate-300 bg-white px-3 text-xs hover:bg-slate-100">
            {busy ? <><Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" /> Importing…</> : "Choose file"}
          </span>
        </label>
      </div>
