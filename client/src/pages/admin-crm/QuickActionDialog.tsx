...
export default function QuickActionDialog({
  ...
  const validitySelect = (value: string, onChange: (v: string) => void) => (
    <select
      ...
      className="w-full rounded-md bg-black/30 border border-white/10 text-sm px-3 py-2"
    >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={onClose}>
      <div
        className="w-full max-w-xl rounded-2xl border border-white/10 bg-[#0d1526] text-slate-100 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between">
          <div className="font-semibold flex items-center gap-2">
            <Plus className="h-4 w-4 text-indigo-300" /> Quick Action
          </div>
          <button className="text-slate-400 hover:text-white text-sm" onClick={onClose}>
            Close
          </button>
        </div>
          ...
          {TABS.map((t) => (
            <button
              ...
              className={`whitespace-nowrap rounded-full px-3 py-1.5 text-xs border ${
                tab === t.id ? "bg-indigo-500/20 border-indigo-400/40 text-white" : "border-white/10 text-slate-300"
              }`}
            >
              {t.label}
            </button>
        ...
        <div className="p-4 space-y-3">
          {(tab === "user" || tab === "healer") && (
            <>
              {!canEditUsers && <p className="text-xs text-amber-200">You need edit permission to create accounts.</p>}
              <Input
          ...
          {tab === "lead" && (
            <>
              {!canEditUsers && <p className="text-xs text-amber-200">You need edit permission to add leads.</p>}
              <Input
                placeholder="Name *"
                value={lead.name}
          ...
          {tab === "ticket" && (
            <>
              {!canManageTickets && <p className="text-xs text-amber-200">You need ticket permission.</p>}
              <Input
                placeholder="Subject *"
          ...
          {tab === "credits" && (
            <>
              {!canEditCredits && <p className="text-xs text-amber-200">You need credit permission.</p>}
              <Input
