...
const DetailedAuraReadingCard = memo(function DetailedAuraReadingCard({ reading }: { reading: any }) {
  ...
  const generateComprehensivePDF = async (reading: any) => {
    ...
    try {
      ...
      // Save the PDF and upload to server for permanent storage
      const timestamp = format(new Date(), 'yyyy-MM-dd');
      const fileName = `aura-chakra-analysis-${reading.name.toLowerCase().replace(/\s+/g, '-')}-${timestamp}.pdf`;
      
      // Get PDF as binary data for server upload
...
function DetailedNumerologyReadingCard({ reading }: { reading: any }) {
  ...
  const downloadPDF = async () => {
    ...
    // Save the PDF and upload to server for permanent storage
    const fileName = `numerology-reading-${reading.name.toLowerCase().replace(/\s+/g, '-')}-${format(new Date(reading.createdAt), "yyyy-MM-dd")}.pdf`;
