...
import demoPdfReport from "@assets/aura-chakra-analysis-vidhu-gupta-2025-11-29.pdf_(8)_1768045427610.pdf";
...
const processImageWithVibeAuraEffect = (
  ...
  img.onload = () => {
    ...
    ctx.save();
    // ctx.filter = 'blur(40px)'; // iOS Canvas filter issue
    ...
    for (let i = 0; i < 300; i++) {
      ...
      // Use radial gradient for soft blur instead of ctx.filter
    ...
    for (let i = 0; i < 600; i++) {
      ...
      const gradient = ctx.createRadialGradient(x, y, 0, x, y, radius);
      gradient.addColorStop(0, `rgba(${r}, ${g}, ${b}, ${opacity})`);
      gradient.addColorStop(0.8, `rgba(${r}, ${g}, ${b}, ${opacity * 0.1})`);
      gradient.addColorStop(1, `rgba(${r}, ${g}, ${b}, 0)`);
