...
export default function AuraAnalysis() {
  ...
  const getGivingEnergyInterpretation = (color: string): string => {
    const givingInterpretations: Record<string, string> = {
      ...
      'teal': 'Heart-throat bridge - healing communication, emotional truth, compassionate expression',
      'Peach': 'Nurturing heart - gentle love energy, emotional healing, compassionate care',
      'Grey': 'Neutral wisdom - spiritual balance, cosmic neutrality, divine equilibrium',
      'Black': 'Shadow integration - transformation power, deep inner work, void consciousness',
      'Brown': 'Earth connection - material stability, physical grounding, natural wisdom',
    };
  ...
  const getReceivingEnergyInterpretation = (color: string): string => {
    const receivingInterpretations: Record<string, string> = {
      ...
      'Teal': 'Receiving Zone Teal - You receive healing communication and emotional truth, attracting compassionate guidance and heart-centered wisdom.',
      'Peach': 'Receiving Zone Peach - You receive gentle care and emotional support, attracting nurturing love and compassionate healing.',
      'Grey': 'Receiving Zone Grey - You receive spiritual balance and wise neutrality, attracting peaceful resolution and adaptable wisdom.',
      ...
      'Cyan': 'Receiving Zone Cyan - You receive emotional clarity and healing communication, attracting pure feeling energy that heals emotional wounds.',
    };
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          // Create mystical smoke particles around the person
          const createMysticalSmoke = () => {
            console.log('Creating mystical smoke effects');
            
            // Create smoke particles around the person
            const smokeCount = 80;
            
            for (let i = 0; i < smokeCount; i++) {
              // Generate positions around the person but not over them
              const angle = (Math.PI * 2 * i) / smokeCount + Math.random() * 0.5;
              const distance = personRadius + 30 + Math.random() * 150;
              
              const smokeX = centerX + Math.cos(angle) * distance;
              const smokeY = centerY + Math.sin(angle) * distance;
              
              // Skip if outside canvas
              if (smokeX < 0 || smokeX > canvas.width || smokeY < 0 || smokeY > canvas.height) {
                continue;
              }
              
              const smokeSize = 20 + Math.random() * 40;
              const useSecondary = Math.random() > 0.6;
              const [r, g, b] = useSecondary ? [sr, sg, sb] : [dr, dg, db];
              const smokeOpacity = 0.15 + Math.random() * 0.20;
              ...
              smokeGradient.addColorStop(0, `rgba(${r}, ${g}, ${b}, ${smokeOpacity})`);
              smokeGradient.addColorStop(0.5, `rgba(${r}, ${g}, ${b}, ${smokeOpacity * 0.6})`);
