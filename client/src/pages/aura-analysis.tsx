...
export default function AuraAnalysis() {
  ...
  const emailMePDF = async () => {
    if (!result) {
      console.error('No aura analysis result available for email PDF generation');
      toast({
        title: "Email PDF Failed",
    ...
    if (!user) {
      toast({
        title: "Email PDF Failed", 
        description: "Please log in to email the PDF to your account.",
        variant: "destructive",
      });
      return;
    }

    const nameToUse = result.analysisName || analysisName || user.username || 'User';
    ...
    try {
      ...
      toast({
        ...
        description: "Creating your comprehensive aura analysis report for email...",
      });

      // Generate PDF similar to download function but for email
      ...
      // Helper functions (reuse from download function)
      const addTextWithPageBreak = (
        ...
        for (let i = 0; i < lines.length; i++) {
          if (y + (i * lineHeight) > pageHeight - 30) {
            pdf.addPage();
            y = 20;
          }
          pdf.text(lines[i], x, y + (i * lineHeight), options);
        }
      ...
      const compressImageForPDF = (
        ...
        compressCanvas.height = targetWidth * aspectRatio;

        ctx.drawImage(canvas, 0, 0, compressCanvas.width, compressCanvas.height);
      ...
      pdf.text('Aura & Chakra Analysis Report', pageWidth/2, 40, { align: 'center' });
      
      pdf.setFontSize(16);
      pdf.setTextColor(100, 100, 100);
      pdf.text(`Prepared for: ${nameToUse}`, pageWidth/2, 55, { align: 'center' });
      
      if (user?.userType === 'healer') {
        pdf.text(`Healer: ${user.username}`, pageWidth/2, 65, { align: 'center' });
      }
      
      pdf.text(`Analysis Date: ${new Date().toLocaleDateString()}`, pageWidth/2, 75, { align: 'center' });

      // Add summary content (abbreviated for email)
      currentY = 100;
      pdf.setFontSize(14);
      ...
      // Prepare screenshots
      const screenshots = Array.from(capturedScreenshots.entries()).map(([tabId, dataUrl]) => ({
        filename: `aura-analysis-${tabId}-screenshot.png`,
        dataUrl: dataUrl
      }));
      ...
      toast({
        title: "Email PDF Failed",
        description: error instanceof Error ? error.message : "Failed to generate and email PDF report. Please try again.",
        variant: "destructive",
      });
    }
  };
                    ...
                    {result && !isAnalyzing && (
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button 
                          variant="outline" 
                          size="sm"
                          className="flex items-center text-xs sm:text-sm bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100"
                          onClick={emailMePDF}
                        >
