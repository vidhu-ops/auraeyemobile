                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    ...
                                    <div className="text-center mb-3">
                                      <div className="text-2xl font-bold text-green-600">
                                        {(() => {
                                          ...
                                          const score = (0.5 * (result.chakraActivity?.root || 5)) + (0.3 * earthStarScore) + (0.2 * (result.chakraActivity?.solarPlexus || 5));
                                          return score.toFixed(1);
                                        })()}/10
                                      </div>
                                      <div className="text-xs text-gray-500">Material stability & wealth magnetism</div>
                                    </div>
                                    <div className="text-xs text-gray-600">
                                      {(() => {
                                        const earthStarScore = calculateEarthStarChakra(result)/10;
                                        const score = (0.5 * (result.chakraActivity?.root || 5)) + (0.3 * earthStarScore) + (0.2 * (result.chakraActivity?.solarPlexus || 5));
                                        if (score <= 4) return "Financial struggles, blocks around money";
                                        if (score <= 6) return "Moderate financial stability, some abundance blocks";
                                        if (score <= 8) return "Good financial flow, growing abundance";
                                        return "Excellent wealth magnetism and financial stability";
                                      })()}
                                    </div>
                                  </div>
                                  ...
                                  <div className="bg-white rounded-lg p-4 border border-yellow-200">
                                    <div className="flex items-center mb-3">
                                      ...
                                      <h5 className="font-semibold text-sm text-yellow-800">Career & Purpose</h5>
                                    </div>
                                    <div className="text-center mb-3">
                                      <div className="text-2xl font-bold text-yellow-600">
                                        {(() => {
                                          const soulStarScore = calculateSoulStarChakra(result)/10;
                                          const score = (0.4 * (result.chakraActivity?.solarPlexus || 5)) + (0.3 * soulStarScore) + (0.3 * (result.chakraActivity?.throat || 5));
                                          return score.toFixed(1);
                                        })()}/10
                                      </div>
                                      <div className="text-xs text-gray-500">Life purpose clarity & professional fulfillment</div>
                                    </div>
                                    <div className="text-xs text-gray-600">
                                      {(() => {
                                        const soulStarScore = calculateSoulStarChakra(result)/10;
                                        const score = (0.4 * (result.chakraActivity?.solarPlexus || 5)) + (0.3 * soulStarScore) + (0.3 * (result.chakraActivity?.throat || 5));
                                        if (score <= 4) return "Career confusion, lack of direction";
                                        if (score <= 6) return "Moderate career satisfaction, seeking purpose";
                                        if (score <= 8) return "Good career alignment, growing fulfillment";
                                  ...
                                  <div className="bg-white rounded-lg p-4 border border-blue-200">
                                    <div className="flex items-center mb-3">
                                      ...
                                      <h5 className="font-semibold text-sm text-blue-800">Emotional Stability</h5>
                                    </div>
                                    <div className="text-center mb-3">
                                      <div className="text-2xl font-bold text-blue-600">
                                        {(() => {
                                          const score = (0.4 * (result.chakraActivity?.heart || 5)) + (0.4 * (result.chakraActivity?.sacral || 5)) + (0.2 * (result.chakraActivity?.solarPlexus || 5));
                                          return score.toFixed(1);
                                        })()}/10
                                      </div>
                                      <div className="text-xs text-gray-500">Emotional regulation & inner peace</div>
                                    </div>
                                    <div className="text-xs text-gray-600">
                                      {(() => {
                                        const score = (0.4 * (result.chakraActivity?.heart || 5)) + (0.4 * (result.chakraActivity?.sacral || 5)) + (0.2 * (result.chakraActivity?.solarPlexus || 5));
                                        if (score <= 4) return "Emotional turbulence, frequent mood swings";
                                        if (score <= 6) return "Moderate emotional balance, some instability";
                                        if (score <= 8) return "Good emotional regulation, stable moods";
                                        return "Excellent emotional stability and inner peace";
                                      })()}
                                    </div>
                                  </div>
                                  ...
                                  <div className="bg-white rounded-lg p-4 border border-purple-200">
                                    <div className="flex items-center mb-3">
                                      ...
                                      <h5 className="font-semibold text-sm text-purple-800">Spiritual Growth</h5>
                                    </div>
                                    <div className="text-center mb-3">
                                      <div className="text-2xl font-bold text-purple-600">
                                        {(() => {
                                          const soulStarScore = calculateSoulStarChakra(result)/10;
                                          const score = (0.4 * (result.chakraActivity?.crown || 5)) + (0.3 * soulStarScore) + (0.3 * (result.chakraActivity?.thirdEye || 5));
                                          return score.toFixed(1);
                                        })()}/10
                                      </div>
                                      <div className="text-xs text-gray-500">Spiritual connection & consciousness expansion</div>
                                    </div>
                                    <div className="text-xs text-gray-600">
                                      {(() => {
                                        const soulStarScore = calculateSoulStarChakra(result)/10;
                                        const score = (0.4 * (result.chakraActivity?.crown || 5)) + (0.3 * soulStarScore) + (0.3 * (result.chakraActivity?.thirdEye || 5));
                                        if (score <= 4) return "Spiritual disconnection, lack of higher purpose";
                                        if (score <= 6) return "Moderate spiritual awareness, seeking connection";
                                        if (score <= 8) return "Good spiritual connection, growing consciousness";
                                        return "Excellent spiritual growth and divine connection";
                                      })()}
                                    </div>
                                  </div>
                                  ...
                                  <div className="bg-white rounded-lg p-4 border border-orange-200">
                                    ...
                                    <div className="text-center mb-3">
                                      <div className="text-2xl font-bold text-orange-600">
                                        {(() => {
                                          const earthStarScore = calculateEarthStarChakra(result)/10;
                                          const score = (0.5 * (result.chakraActivity?.root || 5)) + (0.3 * earthStarScore) + (0.2 * (result.chakraActivity?.sacral || 5));
                                          return score.toFixed(1);
                                        })()}/10
                                      </div>
                                      <div className="text-xs text-gray-500">Physical vitality & life force energy</div>
                                    </div>
                                    <div className="text-xs text-gray-600">
                                      {(() => {
                                        const earthStarScore = calculateEarthStarChakra(result)/10;
                                        const score = (0.5 * (result.chakraActivity?.root || 5)) + (0.3 * earthStarScore) + (0.2 * (result.chakraActivity?.sacral || 5));
                                        if (score <= 4) return "Low physical energy, chronic fatigue";
